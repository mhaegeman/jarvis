...
## [2026-04-14] ingest | obra/superpowers — Skills-Based Coding Agent Workflow Plugin

**Operation:** ingest
**Summary:** Ingested GitHub repo obra/superpowers via repomix (docs + markdown only; 564KB / 132K tokens, 90 files — large repo, read in batches). Superpowers is a complete skills-based workflow plugin for AI coding agents, built and maintained by Jesse Vincent / Prime Radiant. MIT-licensed, zero-dependency, v5.0.7. Available on the official Claude Code plugin marketplace (`/plugin install superpowers@claude-plugins-official`) and via manual install on Cursor, Codex, OpenCode, Gemini CLI, and GitHub Copilot CLI. Core mechanism: a session-start shell hook injects the `using-superpowers` skill at session startup, which obliges the agent to invoke the relevant skill before ANY action (1% rule). 13 skills enforce a mandatory development lifecycle: `brainstorming` (HARD-GATE: no code before approved spec), `using-git-worktrees`, `writing-plans` (checkbox tasks, exact file paths), `subagent-driven-development` (fresh subagent per task + two-stage review: spec compliance then code quality; implementer statuses: DONE/DONE_WITH_CONCERNS/BLOCKED/NEEDS_CONTEXT), `test-driven-development`, `requesting-code-review`, `receiving-code-review`, `systematic-debugging`, `verification-before-completion` (Iron Law: no claims without fresh evidence; 24 failure memories), `finishing-a-development-branch`, `dispatching-parallel-agents`, `writing-skills`, `using-superpowers`. Key v5.0.6 change: inline self-review replaces subagent review loops for specs/plans (same quality, 25 min faster). Visual brainstorming companion: zero-dep Node.js WS server + browser window (opt-in). Priority hierarchy: user CLAUDE.md > skills > system prompt. `<SUBAGENT-STOP>` block prevents recursive skill activation in dispatched subagents. Notable: 94% PR rejection rate, with explicit CLAUDE.md warning to AI agents about low-quality submissions. Created 1 source page, 3 entity pages (Superpowers, Prime Radiant, Jesse Vincent as person), 2 new concept pages (Skills-Based Agent Extension, Subagent-Driven Development). Updated Claude Code entity, Agentic Workflow Patterns concept, overview.md (Theme 7 extended + 3 new open questions), hot.md, index.md (88 pages).
**Pages touched:** [sources/obra-superpowers.md](sources/obra-superpowers.md), [entities/superpowers.md](entities/superpowers.md), [entities/prime-radiant.md](entities/prime-radiant.md), [people/jesse-vincent.md](people/jesse-vincent.md), [concepts/skills-based-agent-extension.md](concepts/skills-based-agent-extension.md), [concepts/subagent-driven-development-concept.md](concepts/subagent-driven-development-concept.md), [entities/claude-code.md](entities/claude-code.md), [concepts/agentic-workflow-patterns.md](concepts/agentic-workflow-patterns.md), [hot.md](hot.md), [overview.md](overview.md), [index.md](index.md)
