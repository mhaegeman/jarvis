...
export class NotifManager {
  ...
  update(inputs: NotifInputs): CompassNotif[] {
    ...
    if (tasks) {
      ...
      if (prev) {
        const startedCount = Math.max(0, tasks.active - prev.active) +
          // Tasks that went queue→run→done in one tick: at minimum, count diff between
          // done deltas not explained by run drain. Approximation: a started chip per
          // (done_delta - max(0, prev.active - tasks.active)) when active stayed flat.
          0;
    ...
    // Assign angles and keyboard shortcuts in display order.
    const display = this.snapshot();
    return display.map((chip, idx) => ({
      ...chip,
      angle: CHIP_ANGLES[idx % CHIP_ANGLES.length],
      kbd: idx < 6 ? `Ctrl ${idx + 1}` : "",
    }));
  }
